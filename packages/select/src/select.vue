<template>
  <div
    class="sp-select"
    :class="[selectSize ? 'sp-select--' + selectSize : '', {
      'sp-select--borderless': !bordered
    }]"
    ...
    <div
      ...
      <span v-if="collapseTags && selected.length">
        <el-tag
          ...
          disable-transitions>
          <span class="sp-select__tags-text">{{ item.currentLabel }}</span>
        </el-tag>
        <el-tag
          v-if="selected.length > collapseTagsShowLimit"
          :closable="false"
          :size="collapseTagSize"
          disable-transitions>
          <span class="sp-select__tags-text">+ {{ selected.length - collapseTagsShowLimit }}</span>
        </el-tag>
      </span>
    ...
    <el-input
      ...
      <template slot="suffix">
        <i v-show="!showClose" :class="['sp-select__caret', 'sp-select__icon-one', 'sp-input__icon', 'sp-icon-' + iconClass]"></i>
        <i v-if="showClose" class="sp-select__caret sp-input__icon sp-icon-close-circle" @click="handleClearClick"></i>
      </template>
    </el-input>
    <transition
      name="sp-zoom-in-top"
      ...
      <el-select-menu
        ...
        <el-scrollbar
          ...
          <div v-if="showSelectAll && multiple" class="sp-select-dropdown__list-operate">
            <span class="sp-select-dropdown__select-all" @click.stop="selectAll">全选</span>
            <span class="sp-select-dropdown__deselect-all" @click.stop="deSelectAll">取消全选</span>
          </div>
          <slot></slot>
        </el-scrollbar>
        <template v-if="emptyText && (!allowCreate || loading || (allowCreate && options.length === 0 ))">
          <slot name="empty" v-if="$slots.empty"></slot>
          <p class="sp-select-dropdown__empty" v-else>
            {{ emptyText }}
          </p>
        </template>
      </el-select-menu>
    ...
    methods: {
      ...
      scrollToOption(option) {
        ...
        if (this.$refs.popper && target) {
          const menu = this.$refs.popper.$el.querySelector('.sp-select-dropdown__wrap');
