...
import AriaUtils from '@fe/element-sp-ui/src/utils/aria-utils';
...
import {
  ...
  isEmpty,
  valueEquals
} from '@fe/element-sp-ui/src/utils/util';
...
const getSibling = (el, distance) => {
  const { parentNode } = el;
  if (parentNode) {
    const siblings = parentNode.querySelectorAll('.sp-cascader-node[tabindex="-1"]');
    ...
    handleKeyDown(e) {
      ...
      switch (keyCode) {
        ...
        case KeyCode.left:
          ...
          if (preMenu) {
            const expandedNode = preMenu.$el.querySelector('.sp-cascader-node[aria-expanded="true"]');
        ...
        case KeyCode.right:
          ...
          if (nextMenu) {
            const firstNode = nextMenu.$el.querySelector('.sp-cascader-node[tabindex="-1"]');
            focusNode(firstNode);
          }
          break;
        case KeyCode.enter:
    ...
    */
    ...
    scrollIntoView() {
      ...
      menus.forEach(menu => {
        ...
        if (menuElement) {
          const container = menuElement.querySelector('.sp-scrollbar__wrap');
          const activeNode = menuElement.querySelector('.sp-cascader-node.is-active') ||
            menuElement.querySelector('.sp-cascader-node.in-active-path');
