<template>
  <table @click="handleQuarterTableClick" @mousemove="handleMouseMove" class="sp-quarter-table">
    <tbody>
      <tr v-for="(row, key) in rows" :key="key">
        <td :class="getCellStyle(cell)" v-for="(cell, key) in row" :key="key">
          <div>
            <a class="cell">{{ t('el.datepicker.quarters.' + quarters[cell.text]) }}</a>
          </div>
        </td>
      </tr>
    </tbody>
...
<script type="text/babel">
  ...
  import { isDate, range, getDayCountOfMonth, nextDate, parseQuarterFormatDate } from '../../../../src/utils/date-util';
  ...
  export default {
    ...
    methods: {
      ...
      getCellStyle(cell) {
        ...
        const toquarter = this.getQuarter(today.getMonth());
        const defaultValue = this.defaultValue ? Array.isArray(this.defaultValue) ? this.defaultValue : [this.defaultValue] : [];
        style.disabled = typeof this.disabledDate === 'function' && datesInMonth(year, month).every(this.disabledDate);
        style.current = arrayFindIndex(coerceTruthyValueToArray(this.value), date => {
          if (typeof date === 'string') {
            date = new Date(parseQuarterFormatDate(date));
          }
          return date.getFullYear() === year && this.getQuarter(date.getMonth()) === cell.text;
        }) >= 0;
        style.today = today.getFullYear() === year && toquarter === cell.text;
